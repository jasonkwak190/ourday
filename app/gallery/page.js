...
import { QrCode, Copy, Check, Image, Users, ExternalLink, Trash2, RefreshCw } from 'lucide-react';
...
export default function GalleryPage() {
  ...
  const [selected, setSelected] = useState(null); // 전체화면 사진
  ...
  return (
    <div className="page-wrapper">
      ...
      <div className="card mb-4">
        <div className="flex items-center gap-3 mb-4">
          <div className="flex items-center justify-center rounded-2xl"
            style={{ width: 40, height: 40, backgroundColor: 'var(--toss-blue-light)', flexShrink: 0 }}>
          ...
          <div>
            ...
            <p className="text-xs" style={{ color: 'var(--toss-text-tertiary)' }}>
              QR 코드를 테이블에 올려두면 하객이 바로 사진을 업로드해요
            </p>
          </div>
        </div>
        ...
        {event && (
          <div className="flex flex-col items-center mb-4">
            <div className="rounded-2xl p-3 mb-3"
              style={{ backgroundColor: 'white', border: '1px solid var(--toss-border)' }}>
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(guestUrl)}&bgcolor=ffffff&color=1a1a1a&margin=0`}
                alt="QR 코드"
                width={180}
                height={180}
            ...
            <p className="text-xs tabular-nums px-3 py-1.5 rounded-xl"
              style={{ backgroundColor: 'var(--toss-bg)', color: 'var(--toss-text-secondary)', wordBreak: 'break-all', textAlign: 'center' }}>
              {guestUrl}
            </p>
          </div>
        )}
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            style={{
              ...
              border: '1px solid var(--toss-border)', cursor: 'pointer', fontWeight: 600, fontSize: 14,
            }}>
      ...
      <div className="card mb-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <Image size={16} color="var(--toss-blue)" />
            <span className="text-sm font-bold" style={{ color: 'var(--toss-text-primary)' }}>
              수집된 사진
            </span>
          </div>
          <div className="flex items-center gap-2">
            <button
              onClick={() => event && loadGallery(event.id)}
              disabled={refreshing}
            ...
            <span className="text-sm" style={{ color: 'var(--toss-text-tertiary)' }}>
              / {maxPhotos}장
            </span>
          </div>
        </div>
        ...
        <div className="rounded-full mb-3 overflow-hidden"
          style={{ height: 6, backgroundColor: 'var(--toss-border)' }}>
        ...
        {pct >= 80 && (
          <p className="text-xs" style={{ color: pct >= 100 ? 'var(--toss-red)' : 'var(--toss-yellow)' }}>
            {pct >= 100 ? '무료 한도에 도달했어요. 프리미엄으로 업그레이드하면 무제한으로 받을 수 있어요.' : `슬슬 한도에 가까워지고 있어요 (${maxPhotos - photoCount}장 남음)`}
          </p>
        )}
      </div>
      ...
      {photoCount === 0 ? (
        ...
        <div className="card mb-4">
          <p className="text-sm font-bold mb-3" style={{ color: 'var(--toss-text-primary)' }}>
            전체 사진
          </p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 4 }}>
            {photos.map(photo => (
              <div key={photo.id} className="relative"
                style={{ aspectRatio: '1', borderRadius: 8, overflow: 'hidden', backgroundColor: 'var(--toss-bg)', cursor: 'pointer' }}
                onClick={() => setSelected(photo)}>
                {photo.url ? (
                  // eslint-disable-next-line @next/next/no-img-element
                  <img src={photo.url} alt="하객 사진" style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                ) : (
                  <div className="flex items-center justify-center w-full h-full">
                    <Image size={20} color="var(--toss-text-tertiary)" />
                  </div>
                )}
                {photo.uploader_name && (
                  <div className="absolute bottom-0 left-0 right-0 px-1.5 py-1"
                    style={{ background: 'linear-gradient(transparent, rgba(0,0,0,0.5))' }}>
                    <p className="text-xs font-medium" style={{ color: 'white', fontSize: 10 }}>
                      {photo.uploader_name}
                    </p>
                  </div>
                )}
