  ...
  return (
    <div className="card flex flex-col gap-3">
      ...
      <div className="flex gap-2">
        ...
        <button className="btn-rose flex-1" onClick={onSave} disabled={saving}>{saving ? '저장 중...' : '저장'}</button>
      </div>
    </div>
  );
}
...
/**
 * VendorsTab — 업체 관리 탭 컴포넌트
 * Budget 페이지의 두 번째 탭으로 사용
 * @param {{ coupleId: string }} props
 */
export default function VendorsTab({ coupleId }) {
  ...
  useEffect(() => {
    if (!coupleId) return;
    const load = async () => {
      const { data } = await supabase.from('vendors').select('*').eq('couple_id', coupleId).order('created_at');
      setVendors(data || []);
      setLoading(false);
    };
    load();
  ...
  useEffect(() => {
    if (!coupleId) return;
    const ch = supabase.channel(`vendors-tab-${coupleId}`)
  ...
  function startEdit(vendor) {
    ...
    setEditForm({ type: vendor.type, name: vendor.name, contact_name: vendor.contact_name || '', contact_phone: vendor.contact_phone || '', deposit: vendor.deposit != null ? String(vendor.deposit) : '', balance: vendor.balance != null ? String(vendor.balance) : '', balance_due: vendor.balance_due || '', contract_status: vendor.contract_status || 'candidate', memo: vendor.memo || '' });
    setMenuId(null);
  }
  ...
  async function handleEdit() {
    ...
    setSaving(true);
    const updates = { type: editForm.type, name: editForm.name.trim(), contact_name: editForm.contact_name.trim() || null, contact_phone: editForm.contact_phone.trim() || null, deposit: editForm.deposit ? parseInt(editForm.deposit, 10) : null, balance: editForm.balance ? parseInt(editForm.balance, 10) : null, balance_due: editForm.balance_due || null, contract_status: editForm.contract_status, memo: editForm.memo.trim() || null };
  ...
  const filtered = vendors.filter(v => (filterType === 'all' || v.type === filterType) && (filterStatus === 'all' || v.contract_status === filterStatus));
  ...
  return (
    <div onClick={() => setMenuId(null)}>
      ...
      {urgentVendors.length > 0 && (
        <div className="rounded-2xl px-4 py-3 mb-4 flex items-start gap-2"
          ...
          <div>
            ...
            {urgentVendors.map(v => {
              const diff = Math.ceil((new Date(v.balance_due) - today) / (1000 * 60 * 60 * 24));
              return <p key={v.id} className="text-xs mt-0.5" style={{ color: 'var(--ink-soft)' }}>{v.name} — {diff === 0 ? '오늘!' : `D-${diff}`} ({v.balance?.toLocaleString()}만원)</p>;
            })}
          </div>
        </div>
      )}
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 mb-2" style={{ scrollbarWidth: 'none' }}>
        <button onClick={() => setFilterType('all')} className="flex-shrink-0 px-3 py-1.5 rounded-xl text-xs font-medium"
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-3" style={{ scrollbarWidth: 'none' }}>
        <button onClick={() => setFilterStatus('all')} className="flex-shrink-0 px-3 py-1.5 rounded-xl text-xs font-medium"
          style={{ backgroundColor: filterStatus === 'all' ? 'var(--ink)' : 'var(--beige)', color: filterStatus === 'all' ? 'white' : 'var(--stone)', border: 'none' }}>전체</button>
        {CONTRACT_STATUS.map(s => (
          <button key={s.value} onClick={() => setFilterStatus(s.value)} className="flex-shrink-0 px-3 py-1.5 rounded-xl text-xs font-medium"
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs mt-0.5" style={{ color: 'var(--stone)' }}>
                      {typeInfo.label}{vendor.contact_name && ` · ${vendor.contact_name}`}{vendor.contact_phone && ` · ${vendor.contact_phone}`}
                    </p>
