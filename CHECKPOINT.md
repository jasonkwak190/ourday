...
| 페이지 | 경로 | 상태 |
|--------|------|------|
| 하객 사진 갤러리 | `/gallery` | ⚠️ SUPABASE_SERVICE_ROLE_KEY Vercel 미설정으로 QR 미작동 |
...
### ERR-008 · SUPABASE_SERVICE_ROLE_KEY Vercel 미설정 ← 🔴 현재 미해결
- **증상**: `/gallery` 에서 "서버 오류" — photo_events 조회 불가 / 랜딩 커플 수 0으로 표시
- **원인**: `SUPABASE_SERVICE_ROLE_KEY` 환경변수가 Vercel에 없음
- **해결**: Vercel → Settings → Environment Variables에 추가 후 Redeploy 필요
  - 값: Supabase 대시보드 → Project Settings → API → `service_role` 시크릿 키
...
| 항목 | 방법 |
|------|------|
| Vercel `SUPABASE_SERVICE_ROLE_KEY` 추가 | Vercel → Settings → Env Variables |
| gallery 페이지 디버그 코드 제거 | 위 작업 후 에러 메시지 표시 코드 정리 |
...
### P0 · 바로 해야 할 것
- [ ] **Vercel 환경변수 설정** — 갤러리/랜딩 모두 의존 (개발 아님, 운영 작업)
- [ ] **갤러리 디버그 코드 정리** — 에러 메시지 표시 코드 제거 (env 설정 후)

### P1 · 핵심 UX 완성
- [ ] **온보딩 개선** — 가입 직후 커플 연동 안내 flow (지금은 갑자기 대시보드로 이동)
- [ ] **청첩장 템플릿 미리보기** — 저장 전 모바일 미리보기 팝업
